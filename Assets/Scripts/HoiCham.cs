...
public class HoiCham : MonoBehaviour {

	private Vector3 m_prevPosition;
	private bool m_isStatic;
	private Animator m_animatorHoiCham;
	...
	void Start () {
		...
		m_isStatic = true;
		m_animatorHoiCham = this.GetComponent<Animator> ();
	}
	...
	void OnCollisionEnter2D(Collision2D col1) {
		...
		if ((col1.gameObject.tag == "Player") && (objPos.y < this.transform.position.y)) {
			m_isStatic = false;
			m_animatorHoiCham.SetBool("isDie", true);
		}
	}
}
